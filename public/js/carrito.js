        ...
        success: function (data) {
            ...
            if (data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    // if()
                    let json = JSON.parse(data[i].json);
                    $("#carrito").append('<table id="table_carrito"></table>');
                    $("#table_carrito").append('<tr id="tr_' + i + '" class="w-100 m-0 p-0"></tr>');
                    $("#tr_"+i).append('<td colspan="3" class="text-center">'+json.nombre_servicio +'</td>');

                    if(json.id_servicio == 11){
                        let arr = json.data
                        for (let j = 0; j < arr.length; j++) {
                            $("#table_carrito").append('<tr id="tr_' + i+'_'+j + '"></tr>');
                            $("#tr_"+i+'_'+j).append('<td class="text-left">'+arr[j]['nombre'] +'</td>');
                            $("#tr_"+i+'_'+j).append('<td class="text-right">800.00</td>');
                            $("#tr_"+i+'_'+j).append('<td class="text-right"><i class="fa fa-trash"></i></td>');
