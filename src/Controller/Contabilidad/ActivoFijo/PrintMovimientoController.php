...
class PrintMovimientoController extends AbstractController
{
    ...
    public function index(EntityManagerInterface $em,Request $request,$id_movimiento)
    {
        ...
        $movimiento_activo_fijo = $em->getRepository(MovimientoActivoFijo::class)->findOneBy(['id'=>$id_movimiento,'id_unidad'=>$unidad]);
        ...
        $data[]= array(
            ...
            'valor_depreciacion_mensal'=>number_format(($movimiento_activo_fijo->getIdActivoFijo()->getIdGrupoActivo()->getPorcientoDepreciaAnno()*$movimiento_activo_fijo->getIdActivoFijo()->getValorReal()/12),2),
            'fundamentacion'=>$movimiento_activo_fijo->getFundamentacion(),
            'usuario'=>$movimiento_activo_fijo->getIdUsuario(),
            ...
            'nro_consecutivo' => $movimiento_activo_fijo->getNroConsecutivo()
        );
