...
class DepositoController extends AbstractController
{
    ...
    public function index(EntityManagerInterface $em, $id_cotizacion, Request $request)
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if (floatval($data['monto']) > round(AuxFunctionsTurismo::getResto($em, $id_cotizacion),2)) {
                $this->addFlash('error', 'El máximo saldo a pagar de la cotización es ' . number_format(AuxFunctionsTurismo::getResto($em, $id_cotizacion),2) . '.');
            ...
            $PagosCotizacion->setMonto(floatval($data['monto']));
        ...
        return $this->render('pasarela_pago/pagos/deposito/index.html.twig', [
            ...
            'form' => $form->createView(),
            'bancos' => $bancos,
            'resto_cotizacion' => round(AuxFunctionsTurismo::getResto($em, $id_cotizacion),2)
        ]);
    }
}
